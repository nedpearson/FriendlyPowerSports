// Temporary CJS wrapper for Phase 9 testing
(async () => {
  const { ActionLayer } = await import('./src/agents/executors/ActionLayer.js');
  const { ActionRegistry } = await import('./src/agents/executors/ActionRegistry.js');
  const { AgentLogger } = await import('./src/agents/audit/AgentLogger.js');
